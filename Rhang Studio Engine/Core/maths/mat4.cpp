...
namespace rse {
	namespace maths {
		...
		mat4 mat4::perspective(float fov, float aspectRatio, float near, float far) {
			...
			float q = 1.0f / tan(toRadians(fov));
		...
		mat4 mat4::transform(const vec3& translation, float angle, const vec3& scale) {
			mat4 result = rotation(angle);

			result.elements[0 + 0 * 4] *= scale.x;
			result.elements[1 + 0 * 4] *= scale.x;

			result.elements[0 + 1 * 4] *= scale.y;
			result.elements[1 + 1 * 4] *= scale.y;
		...
		void mat4::updateTransform(const vec3& translation, float angle, const vec3& scale) {
			float r = toRadians(angle);
			float c = cos(r);
			float s = sin(r);

			elements[0 + 0 * 4] = c * scale.x;
			elements[1 + 0 * 4] = s * scale.x;

			elements[0 + 1 * 4] = -s * scale.y;
			elements[1 + 1 * 4] =  c * scale.y;
